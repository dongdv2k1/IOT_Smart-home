...
var client = mqtt.connect('mqtt://172.20.10.14:1883');
...
con.connect(function (err) {
    ...
    var sql = "CREATE TABLE IF NOT EXISTS sensors11(ID int(10) not null primary key auto_increment, Temperature int(3) not null, Humidity int(3) not null, Light int(5) not null )"
...
client.on('message', function (topic, message) {
    ...
    if (cnt_check == 1) {
        ...
        var sql = "INSERT INTO sensors11 ( Temperature, Humidity, Light) VALUES ( '" + newTemp + "', '" + newHumi + "', '" + newLight + "')"
        con.query(sql, function (err, result) {
            if (err) throw err;
            console.log("Table inserted");
            console.log( newTemp + " " + newHumi + " " + newLight);
        });
        exportCharts(con, io);
        cnt_check = 0;
    }
